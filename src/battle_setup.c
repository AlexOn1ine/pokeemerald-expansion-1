...
{
    CreateMaleMon(&gParty[B_ENEMY][0], SPECIES_RALTS, 5);
    LockPlayerFieldControls();
...
void StartRegiBattle(void)
{
    ...
    species = GetMonData(&gParty[B_ENEMY][0], MON_DATA_SPECIES);
    switch (species)
    {
...
static void DowngradeBadPoison(void)
{
    ...
    {
        if (GetMonData(&gParty[B_PLAYER][i], MON_DATA_SANITY_HAS_SPECIES) && GetMonData(&gParty[B_PLAYER][i], MON_DATA_STATUS) == STATUS1_TOXIC_POISON)
            SetMonData(&gParty[B_PLAYER][i], MON_DATA_STATUS, &status);
    }
}
...
static u16 GetSumOfPlayerPartyLevel(u8 numMons)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        u32 species = GetMonData(&gParty[B_PLAYER][i], MON_DATA_SPECIES_OR_EGG);

        if (species != SPECIES_EGG && species != SPECIES_NONE && GetMonData(&gParty[B_PLAYER][i], MON_DATA_HP) != 0)
        {
            sum += GetMonData(&gParty[B_PLAYER][i], MON_DATA_LEVEL);
            if (--numMons == 0)
                break;
        }
    }
...
u8 GetWildBattleTransition(void)
{
    ...
    u8 enemyLevel = GetMonData(&gParty[B_ENEMY][0], MON_DATA_LEVEL);
...
void BattleSetup_StartTrainerBattle(void)
{
    ...
    if (InBattlePyramid())
    {
        ...
        if (gNoOfApproachingTrainers == 2)
        {
            ...
            ZeroMonData(&gParty[B_ENEMY][1]);
            ZeroMonData(&gParty[B_ENEMY][2]);
            ZeroMonData(&gParty[B_ENEMY][4]);
            ZeroMonData(&gParty[B_ENEMY][5]);
        }
        else
        {
...
static void SaveChangesToPlayerParty(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        {
            gSaveBlock1Ptr->playerParty[i] = gParty[B_PLAYER][j];
            j++;
        }
    }
}
