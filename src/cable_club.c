...
static void Task_LinkupExchangeDataWithLeader(u8 taskId)
{
    ...
    else
    {
        ...
        card->monSpecies[0] = GetMonData(&gParty[B_PLAYER][gSelectedOrderFromParty[0] - 1], MON_DATA_SPECIES, NULL);
        card->monSpecies[1] = GetMonData(&gParty[B_PLAYER][gSelectedOrderFromParty[1] - 1], MON_DATA_SPECIES, NULL);
