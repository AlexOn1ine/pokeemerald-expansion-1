...
{
    return GetMonFriendshipScore(&gParty[B_PLAYER][GetLeadMonIndex()]);
}
...
{
    if (GetMonData(&gParty[B_PLAYER][GetLeadMonIndex()], MON_DATA_COOL) < 200)
        return FALSE;
...
{
    if (GetMonData(&gParty[B_PLAYER][GetLeadMonIndex()], MON_DATA_BEAUTY) < 200)
        return FALSE;
...
{
    if (GetMonData(&gParty[B_PLAYER][GetLeadMonIndex()], MON_DATA_CUTE) < 200)
        return FALSE;
...
{
    if (GetMonData(&gParty[B_PLAYER][GetLeadMonIndex()], MON_DATA_SMART) < 200)
        return FALSE;
...
{
    if (GetMonData(&gParty[B_PLAYER][GetLeadMonIndex()], MON_DATA_TOUGH) < 200)
        return FALSE;
...
{
    return CopyMonFavoritePokeblockName(GetNature(&gParty[B_PLAYER][GetLeadMonIndex()]), gStringVar1);
}
...
{
    return GetMonData(&gParty[B_PLAYER][GetLeadMonIndex()], MON_DATA_EFFORT_RIBBON, NULL);
}
...
void GiveLeadMonEffortRibbon(void)
{
    ...
    ribbonSet = TRUE;
    leadMon = &gParty[B_PLAYER][GetLeadMonIndex()];
...
{
    if (GetMonEVCount(&gParty[B_PLAYER][GetLeadMonIndex()]) >= MAX_TOTAL_EVS)
        return TRUE;
...
{
    u16 monData;
    CreateMon(&gParty[B_PLAYER][0], SPECIES_ZIGZAGOON, 7, USE_RANDOM_IVS, FALSE, 0, OT_ID_PLAYER_ID, 0);
    monData = TRUE;
    SetMonData(&gParty[B_PLAYER][0], MON_DATA_ABILITY_NUM, &monData);
    monData = MOVE_TACKLE;
    SetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE1, &monData);
    monData = MOVE_NONE;
    SetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE2, &monData);
    SetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE3, &monData);
    SetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE4, &monData);
}
...
bool8 IsStarterInParty(void)
{
    ...
    {
        if (GetMonData(&gParty[B_PLAYER][i], MON_DATA_SPECIES_OR_EGG, NULL) == starter)
            return TRUE;
    }
    return FALSE;
}
...
{
    if (!CheckPartyPokerus(gParty[B_PLAYER], (1 << PARTY_SIZE) - 1))
        return FALSE;
...
u8 GetLeadMonIndex(void)
{
    ...
    for (i = 0; i < partyCount; i++)
    {
        if (GetMonData(&gParty[B_PLAYER][i], MON_DATA_SPECIES_OR_EGG, NULL) != SPECIES_EGG
         && GetMonData(&gParty[B_PLAYER][i], MON_DATA_SPECIES_OR_EGG, NULL) != SPECIES_NONE)
            return i;
    }
    return 0;
}
...
{
    return GetMonData(&gParty[B_PLAYER][gSpecialVar_0x8004], MON_DATA_SPECIES_OR_EGG, NULL);
}
...
{
    if (GetMonData(&gParty[B_PLAYER][gSpecialVar_0x8004], MON_DATA_LANGUAGE) != GAME_LANGUAGE)
        return TRUE;

    GetMonData(&gParty[B_PLAYER][gSpecialVar_0x8004], MON_DATA_OT_NAME, gStringVar1);
...
bool8 IsBadEggInParty(void)
{
    ...
    for (i = 0; i < partyCount; i++)
    {
        if (GetMonData(&gParty[B_PLAYER][i], MON_DATA_SANITY_IS_BAD_EGG) == TRUE)
            return TRUE;
    }
...
void BufferVarsForIVRater(void)
{
    ...
    ivStorage[STAT_HP] = GetMonData(&gParty[B_PLAYER][gSpecialVar_0x8004], MON_DATA_HP_IV);
    ivStorage[STAT_ATK] = GetMonData(&gParty[B_PLAYER][gSpecialVar_0x8004], MON_DATA_ATK_IV);
    ivStorage[STAT_DEF] = GetMonData(&gParty[B_PLAYER][gSpecialVar_0x8004], MON_DATA_DEF_IV);
    ivStorage[STAT_SPEED] = GetMonData(&gParty[B_PLAYER][gSpecialVar_0x8004], MON_DATA_SPEED_IV);
    ivStorage[STAT_SPATK] = GetMonData(&gParty[B_PLAYER][gSpecialVar_0x8004], MON_DATA_SPATK_IV);
    ivStorage[STAT_SPDEF] = GetMonData(&gParty[B_PLAYER][gSpecialVar_0x8004], MON_DATA_SPDEF_IV);
...
void ShowNatureGirlMessage(void)
{
    ...
    nature = GetNature(&gParty[B_PLAYER][gSpecialVar_0x8004]);
...
void TrySkyBattle(void)
{
    ...
    {
        struct Pokemon* pokemon = &gParty[B_PLAYER][i];
...
bool32 CheckPartyHasSpecies(u32 givenSpecies)
{
    ...
    for (partyIndex = 0; partyIndex < CalculatePlayerPartyCount(); partyIndex++)
        if (GetMonData(&gParty[B_PLAYER][partyIndex], MON_DATA_SPECIES) == givenSpecies)
            return TRUE;
