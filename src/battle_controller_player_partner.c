...
static void PlayerPartnerHandleChoosePokemon(u32 battler)
{
    ...
    // Switching out
    else if (gBattleStruct->monToSwitchIntoId[battler] >= PARTY_SIZE || !IsValidForBattle(&gParty[B_PLAYER][gBattleStruct->monToSwitchIntoId[battler]]))
    {
        chosenMonId = GetMostSuitableMonToSwitchInto(battler, SWITCH_AFTER_KO);

        if (chosenMonId == PARTY_SIZE || !IsValidForBattle(&gParty[B_PLAYER][chosenMonId])) // just switch to the next mon
        {
            ...
            {
                if (GetMonData(&gParty[B_PLAYER][chosenMonId], MON_DATA_HP) != 0
