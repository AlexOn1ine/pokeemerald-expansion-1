...
{
    u8 eligibility = GetContestEntryEligibility(&gParty[B_PLAYER][gContestMonPartyIndex]);
...
u16 HasMonWonThisContestBefore(void)
{
    ...
    struct Pokemon *mon = &gParty[B_PLAYER][gContestMonPartyIndex];
...
void GiveMonContestRibbon(void)
{
    ...
    {
    case CONTEST_CATEGORY_COOL:
        ribbonData = GetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_COOL_RIBBON);
        if (ribbonData <= gSpecialVar_ContestRank && ribbonData <= CONTEST_RANK_MASTER)
        {
            ribbonData++;
            SetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_COOL_RIBBON, &ribbonData);
            if (GetRibbonCount(&gParty[B_PLAYER][gContestMonPartyIndex]) > NUM_CUTIES_RIBBONS)
                TryPutSpotTheCutiesOnAir(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_COOL_RIBBON);
        }
        break;
    case CONTEST_CATEGORY_BEAUTY:
        ribbonData = GetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_BEAUTY_RIBBON);
        if (ribbonData <= gSpecialVar_ContestRank && ribbonData <= CONTEST_RANK_MASTER)
        {
            ribbonData++;
            SetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_BEAUTY_RIBBON, &ribbonData);
            if (GetRibbonCount(&gParty[B_PLAYER][gContestMonPartyIndex]) > NUM_CUTIES_RIBBONS)
                TryPutSpotTheCutiesOnAir(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_BEAUTY_RIBBON);
        }
        break;
    case CONTEST_CATEGORY_CUTE:
        ribbonData = GetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_CUTE_RIBBON);
        if (ribbonData <= gSpecialVar_ContestRank && ribbonData <= CONTEST_RANK_MASTER)
        {
            ribbonData++;
            SetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_CUTE_RIBBON, &ribbonData);
            if (GetRibbonCount(&gParty[B_PLAYER][gContestMonPartyIndex]) > NUM_CUTIES_RIBBONS)
                TryPutSpotTheCutiesOnAir(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_CUTE_RIBBON);
        }
        break;
    case CONTEST_CATEGORY_SMART:
        ribbonData = GetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_SMART_RIBBON);
        if (ribbonData <= gSpecialVar_ContestRank && ribbonData <= CONTEST_RANK_MASTER)
        {
            ribbonData++;
            SetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_SMART_RIBBON, &ribbonData);
            if (GetRibbonCount(&gParty[B_PLAYER][gContestMonPartyIndex]) > NUM_CUTIES_RIBBONS)
                TryPutSpotTheCutiesOnAir(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_SMART_RIBBON);
        }
        break;
    case CONTEST_CATEGORY_TOUGH:
        ribbonData = GetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_TOUGH_RIBBON);
        if (ribbonData <= gSpecialVar_ContestRank && ribbonData <= CONTEST_RANK_MASTER)
        {
            ribbonData++;
            SetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_TOUGH_RIBBON, &ribbonData);
            if (GetRibbonCount(&gParty[B_PLAYER][gContestMonPartyIndex]) > NUM_CUTIES_RIBBONS)
                TryPutSpotTheCutiesOnAir(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_TOUGH_RIBBON);
        }
        break;
    }
}
...
bool8 GiveMonArtistRibbon(void)
{
    ...
    hasArtistRibbon = GetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_ARTIST_RIBBON);
    if (!hasArtistRibbon
    ...
    {
        hasArtistRibbon = 1;
        SetMonData(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_ARTIST_RIBBON, &hasArtistRibbon);
        if (GetRibbonCount(&gParty[B_PLAYER][gContestMonPartyIndex]) > NUM_CUTIES_RIBBONS)
            TryPutSpotTheCutiesOnAir(&gParty[B_PLAYER][gContestMonPartyIndex], MON_DATA_ARTIST_RIBBON);
