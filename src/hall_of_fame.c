...
static void Task_Hof_InitMonData(u8 taskId)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        {
            sHofMonPtr->mon[i].species = GetMonData(&gParty[B_PLAYER][i], MON_DATA_SPECIES_OR_EGG);
            sHofMonPtr->mon[i].tid = GetMonData(&gParty[B_PLAYER][i], MON_DATA_OT_ID);
            sHofMonPtr->mon[i].isShiny = GetMonData(&gParty[B_PLAYER][i], MON_DATA_IS_SHINY);
            sHofMonPtr->mon[i].personality = GetMonData(&gParty[B_PLAYER][i], MON_DATA_PERSONALITY);
            sHofMonPtr->mon[i].lvl = GetMonData(&gParty[B_PLAYER][i], MON_DATA_LEVEL);
            GetMonData(&gParty[B_PLAYER][i], MON_DATA_NICKNAME, nickname);
