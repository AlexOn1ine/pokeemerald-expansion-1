...
static void RestorePyramidPlayerParty(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        ...
        {
            if (GetMonData(&gSaveBlock1Ptr->playerParty[partyIndex], MON_DATA_SPECIES, NULL) == GetMonData(&gParty[B_PLAYER][j], MON_DATA_SPECIES, NULL))
            {
                for (k = 0; k < MAX_MON_MOVES; k++)
                {
                    for (l = 0; l < MAX_MON_MOVES; l++)
                    {
                        if (GetMonData(&gSaveBlock1Ptr->playerParty[partyIndex], MON_DATA_MOVE1 + l, NULL) == GetMonData(&gParty[B_PLAYER][j], MON_DATA_MOVE1 + k, NULL))
                            break;
                    }
                    if (l == MAX_MON_MOVES)
                        SetMonMoveSlot(&gParty[B_PLAYER][j], MOVE_SKETCH, k);
                }
                gSaveBlock1Ptr->playerParty[partyIndex] = gParty[B_PLAYER][j];
...
void GenerateBattlePyramidWildMon(void)
{
    ...
    id = GetMonData(&gParty[B_ENEMY][0], MON_DATA_SPECIES, NULL) - 1;   // index in table (0-11) -> higher index is lower probability
    ...
    // Set species, name
    SetMonData(&gParty[B_ENEMY][0], MON_DATA_SPECIES, &species);
    StringCopy(name, GetSpeciesName(species));
    SetMonData(&gParty[B_ENEMY][0], MON_DATA_NICKNAME, &name);
    ...
    if (moves != NULL)
    {
        ...
        while (1)
        {
            ...
            if (!MonKnowsMove(&gParty[B_ENEMY][0], id))
            {
                // replace random move
                SetMonData(&gParty[B_ENEMY][0], MON_DATA_MOVE1 + Random() % MAX_MON_MOVES, &id);
                break;
            }
            i++;
            if (i == 20)
    ...
    {
        i = GetMonData(&gParty[B_ENEMY][0], MON_DATA_PERSONALITY, NULL) % 2;
        SetMonData(&gParty[B_ENEMY][0], MON_DATA_ABILITY_NUM, &i);
    }
    else
    {
        i = 0;
        ...
        while (1)
        {
            ...
            for (j = 0; j < NUM_ABILITY_SLOTS; j++)
            {
                if (id == gSpeciesInfo[species].abilities[j])
                {
                    ...
                    SetMonData(&gParty[B_ENEMY][0], MON_DATA_ABILITY_NUM, &id);
                    break;
                }
            }
    ...
    if (gSaveBlock2Ptr->frontier.pyramidWinStreaks[gSaveBlock2Ptr->frontier.lvlMode] >= 140)
    {
        ...
        for (i = 0; i < NUM_STATS; i++)
            SetMonData(&gParty[B_ENEMY][0], MON_DATA_HP_IV + i, &id);
    }
...
void GenerateBattlePyramidWildMon(void)
{
    ...
    id = GetMonData(&gParty[B_ENEMY][0], MON_DATA_SPECIES, NULL) - 1;
    SetMonData(&gParty[B_ENEMY][0], MON_DATA_SPECIES, &wildMons[id].species);
    ...
    case 0:
    case 1:
        SetMonData(&gParty[B_ENEMY][0], MON_DATA_ABILITY_NUM, &wildMons[id].abilityNum);
        break;
    case ABILITY_RANDOM:
    default:
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
        SetMonMoveSlot(&gParty[B_ENEMY][0], wildMons[id].moves[i], i);
