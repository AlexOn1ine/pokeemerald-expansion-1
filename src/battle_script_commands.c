...
static void Cmd_getexp(void)
{
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            {
                if (!IsValidForBattle(&gParty[B_PLAYER][i]))
                    continue;
                if ((1u << i) & sentInBits)
                    viaSentIn++;

                holdEffect = GetMonHoldEffect(&gParty[B_PLAYER][i]);
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            holdEffect = GetMonHoldEffect(&gParty[B_PLAYER][*expMonId]);

            if ((holdEffect != HOLD_EFFECT_EXP_SHARE && !wasSentOut && !IsGen6ExpShareEnabled())
             || GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_SPECIES_OR_EGG) == SPECIES_EGG)
            {
                gBattleScripting.getexpState = 5;
                gBattleStruct->battlerExpReward = 0;
            }
            else if ((gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER && *expMonId >= 3)
                  || GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_LEVEL) == MAX_LEVEL)
            {
                gBattleScripting.getexpState = 5;
                gBattleStruct->battlerExpReward = 0;
                if (B_MAX_LEVEL_EV_GAINS >= GEN_5)
                    MonGainEVs(&gParty[B_PLAYER][*expMonId], gBattleMons[gBattlerFainted].species);
            }
            else
            {
                ...
                if (IsValidForBattle(&gParty[B_PLAYER][*expMonId]))
                {
                    if (wasSentOut)
                        gBattleStruct->battlerExpReward = GetSoftLevelCapExpValue(gParty[B_PLAYER][*expMonId].level, gBattleStruct->expValue);
                    else
                        gBattleStruct->battlerExpReward = 0;
                    ...
                    {
                        gBattleStruct->battlerExpReward += GetSoftLevelCapExpValue(gParty[B_PLAYER][*expMonId].level, gBattleStruct->expShareExpValue);;
                    }
                    ...
                    {
                        u32 growthRate = gSpeciesInfo[GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_SPECIES)].growthRate;
                        u32 currentExp = GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_EXP);
                        u32 levelCap = GetCurrentLevelCap();

                        if (GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_LEVEL) >= levelCap)
                            gBattleStruct->battlerExpReward = 0;
                    ...
                    if (IsTradedMon(&gParty[B_PLAYER][*expMonId]))
                    {
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            currLvl = GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_LEVEL);
            if (GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_HP) && currLvl != MAX_LEVEL)
            {
                gBattleResources->beforeLvlUp->stats[STAT_HP]    = GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_MAX_HP);
                gBattleResources->beforeLvlUp->stats[STAT_ATK]   = GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_ATK);
                gBattleResources->beforeLvlUp->stats[STAT_DEF]   = GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_DEF);
                gBattleResources->beforeLvlUp->stats[STAT_SPEED] = GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_SPEED);
                gBattleResources->beforeLvlUp->stats[STAT_SPATK] = GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_SPATK);
                gBattleResources->beforeLvlUp->stats[STAT_SPDEF] = GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_SPDEF);
    ...
    case 4: // lvl up if necessary
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (gBattleResources->bufferB[expBattler][0] == CONTROLLER_TWORETURNVALUES && gBattleResources->bufferB[expBattler][1] == RET_VALUE_LEVELED_UP)
            {
                ...
                PREPARE_BYTE_NUMBER_BUFFER(gBattleTextBuff2, 3, GetMonData(&gParty[B_PLAYER][*expMonId], MON_DATA_LEVEL));

                BattleScriptPushCursor();
                gLeveledUpInBattle |= 1 << *expMonId;
                gBattlescriptCurrInstr = BattleScript_LevelUp;
                gBattleStruct->battlerExpReward = T1_READ_32(&gBattleResources->bufferB[expBattler][2]);
                AdjustFriendship(&gParty[B_PLAYER][*expMonId], FRIENDSHIP_EVENT_GROW_LEVEL);
                ...
                if (battler != 0xFF)
                {
                    CopyMonLevelAndBaseStatsToBattleMon(battler, &gParty[B_PLAYER][*expMonId]);
...
bool32 NoAliveMonsForPlayer(void)
{
    ...
    for (i = 0; i < maxI; i++)
    {
        if (GetMonData(&gParty[B_PLAYER][i], MON_DATA_SPECIES) && !GetMonData(&gParty[B_PLAYER][i], MON_DATA_IS_EGG)
            && (!(gBattleTypeFlags & BATTLE_TYPE_ARENA) || !(gBattleStruct->arenaLostPlayerMons & (1u << i))))
        {
            HP_count += GetMonData(&gParty[B_PLAYER][i], MON_DATA_HP);
        }

        // Get the number of fainted mons or eggs (not empty slots) in the first three party slots.
        if (i < 3 && ((GetMonData(&gParty[B_PLAYER][i], MON_DATA_SPECIES) && !GetMonData(&gParty[B_PLAYER][i], MON_DATA_HP))
         || GetMonData(&gParty[B_PLAYER][i], MON_DATA_IS_EGG)))
            ineligibleMonsCount++;
    }
...
static bool32 NoAliveMonsForOpponent(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        if (GetMonData(&gParty[B_ENEMY][i], MON_DATA_SPECIES) && !GetMonData(&gParty[B_ENEMY][i], MON_DATA_IS_EGG)
         && (!(gBattleTypeFlags & BATTLE_TYPE_ARENA) || !(gBattleStruct->arenaLostOpponentMons & (1u << i))))
        {
            HP_count += GetMonData(&gParty[B_ENEMY][i], MON_DATA_HP);
        }
    }
...
static void Cmd_handlelearnnewmove(void)
{
    ...
    {
        u32 currLvl = GetMonData(&gParty[B_PLAYER][monId], MON_DATA_LEVEL);

        while (gBattleResources->beforeLvlUp->level <= currLvl)
        {
            learnMove = MonTryLearningNewMoveAtLevel(&gParty[B_PLAYER][monId], cmd->isFirstMove, gBattleResources->beforeLvlUp->level);

            while (learnMove == MON_ALREADY_KNOWS_MOVE)
                learnMove = MonTryLearningNewMoveAtLevel(&gParty[B_PLAYER][monId], FALSE, gBattleResources->beforeLvlUp->level);
    ...
    {
        learnMove = MonTryLearningNewMove(&gParty[B_PLAYER][monId], cmd->isFirstMove);
        while (learnMove == MON_ALREADY_KNOWS_MOVE)
            learnMove = MonTryLearningNewMove(&gParty[B_PLAYER][monId], FALSE);
    }
...
static void Cmd_yesnoboxlearnmove(void)
{
    ...
    case 2:
        if (!gPaletteFade.active)
        {
            ...
            ShowSelectMovePokemonSummaryScreen(gParty[B_PLAYER], gBattleStruct->expGetterMonId, gPartyCount[B_PLAYER] - 1, ReshowBattleScreenAfterMenu, gMoveToLearn);
            gBattleScripting.learnMoveState++;
        }
        break;
    case 3:
    ...
    case 4:
        if (!gPaletteFade.active && gMain.callback2 == BattleMainCB2)
        {
            ...
            {
                u16 move = GetMonData(&gParty[B_PLAYER][gBattleStruct->expGetterMonId], MON_DATA_MOVE1 + movePosition);
                if (IsMoveHM(move))
                {
                ...
                else
                {
                    ...
                    RemoveMonPPBonus(&gParty[B_PLAYER][gBattleStruct->expGetterMonId], movePosition);
                    SetMonMoveSlot(&gParty[B_PLAYER][gBattleStruct->expGetterMonId], gMoveToLearn, movePosition);
...
static void Cmd_getmoneyreward(void)
{
    ...
    else
    {
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            {
                if (GetMonData(&gParty[B_PLAYER][i], MON_DATA_LEVEL) > sPartyLevel)
                    sPartyLevel = GetMonData(&gParty[B_PLAYER][i], MON_DATA_LEVEL);
            }
        }
...
static void DrawLevelUpWindow1(void)
{
    ...
    GetMonLevelUpWindowStats(&gParty[B_PLAYER][gBattleStruct->expGetterMonId], currStats);
...
static void DrawLevelUpBannerText(void)
{
    ...
    struct Pokemon *mon = &gParty[B_PLAYER][gBattleStruct->expGetterMonId];
...
static void Cmd_pickup(void)
{
    ...
    if (!InBattlePike()) // No items in Battle Pike.
    {
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            heldItem = GetMonData(&gParty[B_PLAYER][i], MON_DATA_HELD_ITEM);
            lvlDivBy10 = (GetMonData(&gParty[B_PLAYER][i], MON_DATA_LEVEL)-1) / 10; //Moving this here makes it easier to add in abilities like Honey Gather.
            if (lvlDivBy10 > 9)
                lvlDivBy10 = 9;

            ability = gSpeciesInfo[species].abilities[GetMonData(&gParty[B_PLAYER][i], MON_DATA_ABILITY_NUM)];
                ...
                && (Random() % 10) == 0)
            {
                if (isInPyramid)
                {
                    ...
                    SetMonData(&gParty[B_PLAYER][i], MON_DATA_HELD_ITEM, &heldItem);
                }
                else
                {
                    u32 rand = Random() % 100;
                    ...
                    for (j = 0; j < ARRAY_COUNT(sPickupTable); j++)
                    {
                        ...
                        {
                            SetMonData(&gParty[B_PLAYER][i], MON_DATA_HELD_ITEM, &sPickupTable[j].itemId);
                            break;
                        }
                    }
                }
    ...
    case GIVECAUGHTMON_HANDLE_CHOSEN_MON:
        if (gSelectedMonPartyId != PARTY_SIZE)
        {
            ...
            else
            {
                ...
                if (CopyMonToPC(&gParty[B_PLAYER][gSelectedMonPartyId]) == MON_GIVEN_TO_PC)
                {
                    GetMonNickname(&gParty[B_PLAYER][gSelectedMonPartyId], gStringVar2);
                    StringCopy(gStringVar1, GetBoxNamePtr(GetPCBoxToSendMon()));
                    ZeroMonData(&gParty[B_PLAYER][gSelectedMonPartyId]);
...
void ApplyExperienceMultipliers(s32 *expAmount, u8 expGetterMonId, u8 faintedBattler)
{
    enum ItemHoldEffect holdEffect = GetMonHoldEffect(&gParty[B_PLAYER][expGetterMonId]);

    if (IsTradedMon(&gParty[B_PLAYER][expGetterMonId]))
        *expAmount = (*expAmount * 150) / 100;
    if (holdEffect == HOLD_EFFECT_LUCKY_EGG)
        *expAmount = (*expAmount * 150) / 100;
    if (B_UNEVOLVED_EXP_MULTIPLIER >= GEN_6 && IsMonPastEvolutionLevel(&gParty[B_PLAYER][expGetterMonId]))
        *expAmount = (*expAmount * 4915) / 4096;
    if (B_AFFECTION_MECHANICS == TRUE && GetMonAffectionHearts(&gParty[B_PLAYER][expGetterMonId]) >= AFFECTION_FOUR_HEARTS)
        *expAmount = (*expAmount * 4915) / 4096;
    ...
    if (B_SCALED_EXP >= GEN_5 && B_SCALED_EXP != GEN_6)
    {
        ...
        u8 expGetterLevel = GetMonData(&gParty[B_PLAYER][expGetterMonId], MON_DATA_LEVEL);
...
void BS_ItemRestorePP(void)
{
    ...
    u32 battler = MAX_BATTLERS_COUNT;
    struct Pokemon *mon = (IsOnPlayerSide(gBattlerAttacker)) ? &gParty[B_PLAYER][gBattleStruct->itemPartyIndex[gBattlerAttacker]] : &gParty[B_ENEMY][gBattleStruct->itemPartyIndex[gBattlerAttacker]];
...
void BS_CheckPokeFlute(void)
{
    ...
    UpdatePokeFlutePartyStatus(gParty[B_PLAYER], B_POSITION_PLAYER_LEFT);
    UpdatePokeFlutePartyStatus(gParty[B_ENEMY], B_POSITION_OPPONENT_LEFT);
