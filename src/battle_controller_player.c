...
static void Task_GiveExpToMon(u8 taskId)
{
    ...
    {
        struct Pokemon *mon = &gParty[B_PLAYER][monId];
...
static void Task_PrepareToGiveExpWithExpBar(u8 taskId)
{
    ...
    s32 gainedExp = GetTaskExpValue(taskId);
    u8 battler = gTasks[taskId].tExpTask_battler;
    struct Pokemon *mon = &gParty[B_PLAYER][monIndex];
    u8 level = GetMonData(mon, MON_DATA_LEVEL);
    u16 species = GetMonData(mon, MON_DATA_SPECIES);
...
static void Task_UpdateLvlInHealthbox(u8 taskId)
{
    ...
    if (!gBattleSpritesDataPtr->healthBoxesData[battler].specialAnimActive)
    {
        ...
        if (IsDoubleBattle() == TRUE && monIndex == gBattlerPartyIndexes[BATTLE_PARTNER(battler)])
            UpdateHealthboxAttribute(gHealthboxSpriteIds[BATTLE_PARTNER(battler)], &gParty[B_PLAYER][monIndex], HEALTHBOX_ALL);
        else
            UpdateHealthboxAttribute(gHealthboxSpriteIds[battler], &gParty[B_PLAYER][monIndex], HEALTHBOX_ALL);
...
void PlayerHandleExpUpdate(u32 battler)
{
    ...
    if (GetMonData(&gParty[B_PLAYER][monId], MON_DATA_LEVEL) >= MAX_LEVEL)
    {
        PlayerBufferExecCompleted(battler);
    }
    else
