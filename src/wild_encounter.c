...
static u8 ChooseWildMonLevel(const struct WildPokemon *wildPokemon, u8 wildMonIndex, enum WildPokemonArea area)
{
    ...
    if (LURE_STEP_COUNT == 0)
    {
        ...
        {
            u16 ability = GetMonAbility(&gParty[B_PLAYER][0]);
...
u8 PickWildMonNature(void)
{
    ...
    if (!GetMonData(&gParty[B_PLAYER][0], MON_DATA_SANITY_IS_EGG)
        && GetMonAbility(&gParty[B_PLAYER][0]) == ABILITY_SYNCHRONIZE
        && (OW_SYNCHRONIZE_NATURE >= GEN_8 || Random() % 2 == 0))
    {
        return GetMonData(&gParty[B_PLAYER][0], MON_DATA_PERSONALITY) % NUM_NATURES;
    }
...
void CreateWildMon(u16 species, u8 level)
{
    ...
    if (checkCuteCharm
        && !GetMonData(&gParty[B_PLAYER][0], MON_DATA_SANITY_IS_EGG)
        && GetMonAbility(&gParty[B_PLAYER][0]) == ABILITY_CUTE_CHARM
        && Random() % 3 != 0)
    {
        u16 leadingMonSpecies = GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPECIES);
        u32 leadingMonPersonality = GetMonData(&gParty[B_PLAYER][0], MON_DATA_PERSONALITY);
        ...
        CreateMonWithGenderNatureLetter(&gParty[B_ENEMY][0], species, level, USE_RANDOM_IVS, gender, PickWildMonNature(), 0);
        return;
    }
...
static bool8 SetUpMassOutbreakEncounter(u8 flags)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
        SetMonMoveSlot(&gParty[B_ENEMY][0], gSaveBlock1Ptr->outbreakPokemonMoves[i], i);
...
static bool8 WildEncounterCheck(u32 encounterRate, bool8 ignoreAbility)
{
    ...
    if (LURE_STEP_COUNT != 0)
        encounterRate *= 2;
    if (!ignoreAbility && !GetMonData(&gParty[B_PLAYER][0], MON_DATA_SANITY_IS_EGG))
    {
        u32 ability = GetMonAbility(&gParty[B_PLAYER][0]);
...
static bool8 IsWildLevelAllowedByRepel(u8 wildLevel)
{
    ...
    {
        if (I_REPEL_INCLUDE_FAINTED == GEN_1 || I_REPEL_INCLUDE_FAINTED >= GEN_6 || GetMonData(&gParty[B_PLAYER][i], MON_DATA_HP))
        {
            if (!GetMonData(&gParty[B_PLAYER][i], MON_DATA_IS_EGG))
                return wildLevel >= GetMonData(&gParty[B_PLAYER][i], MON_DATA_LEVEL);
        }
    }
...
static bool8 IsAbilityAllowingEncounter(u8 level)
{
    ...
    {
        u8 playerMonLevel = GetMonData(&gParty[B_PLAYER][0], MON_DATA_LEVEL);
...
#endif
{
    if (GetMonData(&gParty[B_PLAYER][0], MON_DATA_SANITY_IS_EGG))
        return FALSE;
    else if (GetMonAbility(&gParty[B_PLAYER][0]) != ability)
        return FALSE;
    else if (Random() % 2 != 0)
        return FALSE;
...
static void ApplyCleanseTagEncounterRateMod(u32 *encRate)
{
    if (GetMonData(&gParty[B_PLAYER][0], MON_DATA_HELD_ITEM) == ITEM_CLEANSE_TAG)
        *encRate = *encRate * 2 / 3;
}
