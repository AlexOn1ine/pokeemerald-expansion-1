...
u8 CountPartyAliveNonEggMonsExcept(u8 slotToIgnore)
{
    ...
    for (i = 0, count = 0; i < PARTY_SIZE; i++)
    {
        if (i != slotToIgnore
            ...
            && !GetMonData(&gParty[B_PLAYER][i], MON_DATA_IS_EGG)
            && GetMonData(&gParty[B_PLAYER][i], MON_DATA_HP) != 0)
        {
            count++;
        }
    }
...
static void Task_OnCloseBoxPressed(u8 taskId)
{
    ...
    case 4:
        ...
        {
            UpdateBoxToSendMons();
            gPartyCount[B_PLAYER] = CalculatePlayerPartyCount();
...
static void GiveChosenBagItem(void)
{
    ...
    if (itemId != ITEM_NONE)
    {
        ...
        if (sInPartyMenu)
        {
            struct Pokemon *mon = &gParty[B_PLAYER][pos];
            SetMonData(&gParty[B_PLAYER][pos], MON_DATA_HELD_ITEM, &itemId);
...
static void SetPartySlotTilemaps(void)
{
    ...
    {
        s32 species = GetMonData(&gParty[B_PLAYER][i], MON_DATA_SPECIES);
...
{
    u16 i, count;
    u16 species = GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPECIES_OR_EGG);
    u32 personality = GetMonData(&gParty[B_PLAYER][0], MON_DATA_PERSONALITY);
    ...
    for (i = 1; i < PARTY_SIZE; i++)
    {
        ...
        if (species != SPECIES_NONE)
        {
            personality = GetMonData(&gParty[B_PLAYER][i], MON_DATA_PERSONALITY);
    ...
    if (sStorage->boxOption == OPTION_MOVE_ITEMS)
    {
        for (i = 0; i < PARTY_SIZE; i++)
        {
            if (sStorage->partySprites[i] != NULL && GetMonData(&gParty[B_PLAYER][i], MON_DATA_HELD_ITEM) == ITEM_NONE)
...
static u16 GetSpeciesAtCursorPosition(void)
{
    ...
    {
    case CURSOR_AREA_IN_PARTY:
        return GetMonData(&gParty[B_PLAYER][sCursorPosition], MON_DATA_SPECIES);
    case CURSOR_AREA_IN_BOX:
...
static void PlaceMon(void)
{
    ...
    case CURSOR_AREA_IN_PARTY:
        ...
        struct Pokemon *mon = &gParty[B_PLAYER][sCursorPosition];
        UpdateSpeciesSpritePSS(&mon->box);
        break;
    case CURSOR_AREA_IN_BOX:
...
static void SetMovingMonData(u8 boxId, u8 position)
{
    ...
    {
        sStorage->movingMon = gParty[B_PLAYER][sCursorPosition];
        if (&gParty[B_PLAYER][sCursorPosition] == GetFirstLiveMon())
            gFollowerSteps = 0;
    }
    else
    {
...
static void SetPlacedMonData(u8 boxId, u8 position)
{
    ...
    if (boxId == TOTAL_BOXES_COUNT)
    {
        gParty[B_PLAYER][position] = sStorage->movingMon;
        struct Pokemon *mon = &gParty[B_PLAYER][position];
...
{
    if (boxId == TOTAL_BOXES_COUNT)
        ZeroMonData(&gParty[B_PLAYER][position]);
    else
...
{
    if (boxId == TOTAL_BOXES_COUNT)
        sStorage->tempMon = gParty[B_PLAYER][position];
    else
    ...
    else
    {
        if (sCursorArea == CURSOR_AREA_IN_PARTY)
        {
            ...
            if (OW_PC_RELEASE_ITEM >= GEN_8)
                item = GetMonData(&gParty[B_PLAYER][sCursorPosition], MON_DATA_HELD_ITEM);
        }
        else
        {
            boxId = StorageGetCurrentBox();
...
static void InitCanReleaseMonVars(void)
{
    ...
    else
    {
        if (sCursorArea == CURSOR_AREA_IN_PARTY)
        {
            sStorage->tempMon = gParty[B_PLAYER][sCursorPosition];
...
static bool32 AtLeastThreeUsableMons(void)
{
    ...
    {
        if (GetMonData(&gParty[B_PLAYER][j], MON_DATA_SANITY_HAS_SPECIES))
            count++;
    }
...
static s8 RunCanReleaseMon(void)
{
    ...
    case 0:
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            {
                knownMoves = GetMonData(&gParty[B_PLAYER][i], MON_DATA_KNOWN_MOVES, (u8 *)sStorage->restrictedMoveList);
...
static void InitSummaryScreenData(void)
{
    ...
    {
        sStorage->summaryMon.mon = gParty[B_PLAYER];
...
s16 CompactPartySlots(void)
{
    ...
    {
        u16 species = GetMonData(&gParty[B_PLAYER][i], MON_DATA_SPECIES);
        if (species != SPECIES_NONE)
        {
            if (i != last)
                gParty[B_PLAYER][last] = gParty[B_PLAYER][i];
            last++;
        }
        else if (retVal == -1)
        {
    ...
    for (; last < PARTY_SIZE; last++)
        ZeroMonData(&gParty[B_PLAYER][last]);
...
static void SetMonMarkings(u8 markings)
{
    ...
    {
        if (sCursorArea == CURSOR_AREA_IN_PARTY)
            SetMonData(&gParty[B_PLAYER][sCursorPosition], MON_DATA_MARKINGS, &markings);
        if (sCursorArea == CURSOR_AREA_IN_BOX)
...
static bool8 CanPlaceMon(void)
{
    if (sIsMonBeingMoved)
    {
        if (sCursorArea == CURSOR_AREA_IN_PARTY && GetMonData(&gParty[B_PLAYER][sCursorPosition], MON_DATA_SPECIES) == SPECIES_NONE)
            return TRUE;
...
static void TryRefreshDisplayMon(void)
{
    ...
    if (!sIsMonBeingMoved)
    {
        ...
        case CURSOR_AREA_IN_PARTY:
            ...
            {
                SetDisplayMonData(&gParty[B_PLAYER][sCursorPosition], MODE_PARTY);
                break;
            }
            // fallthrough
        case CURSOR_AREA_BUTTONS:
...
static void TryLoadItemIconAtPos(u8 cursorArea, u8 cursorPos)
{
    ...
    case CURSOR_AREA_IN_BOX:
        ...
        break;
    case CURSOR_AREA_IN_PARTY:
        if (cursorPos >= PARTY_SIZE || !GetMonData(&gParty[B_PLAYER][cursorPos], MON_DATA_SANITY_HAS_SPECIES))
            return;
        heldItem = GetMonData(&gParty[B_PLAYER][cursorPos], MON_DATA_HELD_ITEM);
        break;
    default:
        return;
    }
...
static void TakeItemFromMon(u8 cursorArea, u8 cursorPos)
{
    ...
    {
        struct Pokemon *mon = &gParty[B_PLAYER][cursorPos];
        SetMonData(&gParty[B_PLAYER][cursorPos], MON_DATA_HELD_ITEM, &itemId);
...
static void SwapItemsWithMon(u8 cursorArea, u8 cursorPos)
{
    ...
    {
        struct Pokemon *mon = &gParty[B_PLAYER][cursorPos];
        itemId = GetMonData(&gParty[B_PLAYER][cursorPos], MON_DATA_HELD_ITEM);
        SetMonData(&gParty[B_PLAYER][cursorPos], MON_DATA_HELD_ITEM, &sStorage->movingItemId);
        sStorage->movingItemId = itemId;
        SetMonFormPSS(&mon->box, FORM_CHANGE_ITEM_HOLD);
        UpdateSpeciesSpritePSS(&mon->box);
    }
