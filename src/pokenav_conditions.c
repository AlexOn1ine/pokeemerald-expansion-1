...
static u32 OpenMarkingsMenu(struct Pokenav_ConditionMenu *menu)
{
    ...
    if (!HandleMonMarkingsMenuInput())
    {
        ...
        if (boxId == TOTAL_BOXES_COUNT)
            SetMonData(&gParty[B_PLAYER][monId], MON_DATA_MARKINGS, &markings);
        else
...
static u8 *CopyConditionMonNameGender(u8 *str, u16 listId, bool8 skipPadding)
{
    ...
    *(str++) = TEXT_COLOR_LIGHT_BLUE;
    ...
    if (boxId == TOTAL_BOXES_COUNT)
    {
        level = GetMonData(&gParty[B_PLAYER][monId], MON_DATA_LEVEL);
        gender = GetMonGender(&gParty[B_PLAYER][monId]);
    }
    else
    {
