...
bool8 SetUpFieldMove_SoftBoiled(void)
{
    ...
    maxHp = GetMonData(&gParty[B_PLAYER][GetCursorSelectionMonId()], MON_DATA_MAX_HP);
    hp = GetMonData(&gParty[B_PLAYER][GetCursorSelectionMonId()], MON_DATA_HP);
...
void Task_TryUseSoftboiledOnPartyMon(u8 taskId)
{
    ...
    hp = GetMonData(&gParty[B_PLAYER][recipientPartyId], MON_DATA_HP);
    if(hp == 0 || userPartyId == recipientPartyId || GetMonData(&gParty[B_PLAYER][recipientPartyId], MON_DATA_MAX_HP) == hp)
    {
    ...
    PlaySE(SE_USE_ITEM);
    PartyMenuModifyHP(taskId, userPartyId, -1, GetMonData(&gParty[B_PLAYER][userPartyId], MON_DATA_MAX_HP)/5, Task_SoftboiledRestoreHealth);
}

static void Task_SoftboiledRestoreHealth(u8 taskId)
{
    PlaySE(SE_USE_ITEM);
    PartyMenuModifyHP(taskId, gPartyMenu.slotId2, 1, GetMonData(&gParty[B_PLAYER][gPartyMenu.slotId], MON_DATA_MAX_HP)/5, Task_DisplayHPRestoredMessage);
}
