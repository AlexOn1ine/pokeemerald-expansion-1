...
{
    u32 atk;
    CreateMon(&gParty[B_PLAYER][0], SPECIES_WOBBUFFET, 100, 0, FALSE, 0, OT_ID_PRESET, 0);
    atk = GetMonData(&gParty[B_PLAYER][0], MON_DATA_ATK);
    ...
    EXPECT(GetMonData(&gParty[B_PLAYER][0], MON_DATA_HYPER_TRAINED_ATK));
    EXPECT_EQ(atk + 31, GetMonData(&gParty[B_PLAYER][0], MON_DATA_ATK));
    EXPECT(!VarGet(VAR_RESULT));
}
...
TEST("hasgigantamaxfactor/togglegigantamaxfactor affect MON_DATA_GIGANTAMAX_FACTOR")
{
    ...
    EXPECT(VarGet(VAR_RESULT));
    EXPECT(GetMonData(&gParty[B_PLAYER][0], MON_DATA_GIGANTAMAX_FACTOR));
...
{
    CreateMon(&gParty[B_PLAYER][0], SPECIES_MELMETAL, 100, 0, FALSE, 0, OT_ID_PRESET, 0);
...
TEST("givemon [simple]")
{
    ...
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPECIES), SPECIES_WOBBUFFET);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_LEVEL), 100);
}
...
TEST("givemon respects perfectIVCount")
{
    ...
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPECIES), SPECIES_MEW);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][1], MON_DATA_SPECIES), SPECIES_CELEBI);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][2], MON_DATA_SPECIES), SPECIES_JIRACHI);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][3], MON_DATA_SPECIES), SPECIES_MANAPHY);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][4], MON_DATA_SPECIES), SPECIES_VICTINI);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][5], MON_DATA_SPECIES), SPECIES_DIANCIE);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_LEVEL), 100);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][1], MON_DATA_LEVEL), 100);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][2], MON_DATA_LEVEL), 100);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][3], MON_DATA_LEVEL), 100);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][4], MON_DATA_LEVEL), 100);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][5], MON_DATA_LEVEL), 100);
    for (u32 j = 0; j < 6; j++)
    {
        for (u32 k = 0; k < NUM_STATS; k++)
        {
            if (GetMonData(&gParty[B_PLAYER][j], MON_DATA_HP_IV + k) == MAX_PER_STAT_IVS)
                perfectIVs[j]++;
        }
...
TEST("givemon respects FORM_CHANGE_ITEM_HOLD")
{
    ...
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPECIES), SPECIES_ARCEUS_ELECTRIC);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][1], MON_DATA_SPECIES), SPECIES_ARCEUS_ELECTRIC);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][2], MON_DATA_SPECIES), SPECIES_ARCEUS_ELECTRIC);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][3], MON_DATA_SPECIES), SPECIES_GIRATINA_ALTERED);
}
...
TEST("givemon [moves]")
{
    ...
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPECIES), SPECIES_WOBBUFFET);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_LEVEL), 100);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE1), MOVE_SCRATCH);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE2), MOVE_SPLASH);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE3), MOVE_NONE);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE4), MOVE_NONE);
}
...
TEST("givemon [all]")
{
    ...
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPECIES), SPECIES_WOBBUFFET);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_LEVEL), 100);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_HELD_ITEM), ITEM_LEFTOVERS);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_POKEBALL), BALL_MASTER);
    EXPECT_EQ(GetNature(&gParty[B_PLAYER][0]), NATURE_BOLD);
    EXPECT_EQ(GetMonAbility(&gParty[B_PLAYER][0]), gSpeciesInfo[SPECIES_WOBBUFFET].abilities[2]);
    EXPECT_EQ(GetMonGender(&gParty[B_PLAYER][0]), MON_MALE);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_HP_EV), 1);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_ATK_EV), 2);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_DEF_EV), 3);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPEED_EV), 4);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPATK_EV), 5);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPDEF_EV), 6);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_HP_IV), 7);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_ATK_IV), 8);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_DEF_IV), 9);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPEED_IV), 10);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPATK_IV), 11);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_SPDEF_IV), 12);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE1), MOVE_SCRATCH);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE2), MOVE_SPLASH);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE3), MOVE_CELEBRATE);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_MOVE4), MOVE_EXPLOSION);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_IS_SHINY), TRUE);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_GIGANTAMAX_FACTOR), TRUE);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_TERA_TYPE), TYPE_FIRE);
    EXPECT_EQ(GetMonData(&gParty[B_PLAYER][0], MON_DATA_DYNAMAX_LEVEL), 7);
}
...
TEST("createmon [simple]")
{
    ...
    EXPECT_EQ(GetMonData(&gParty[B_ENEMY][0], MON_DATA_SPECIES), SPECIES_WOBBUFFET);
    EXPECT_EQ(GetMonData(&gParty[B_ENEMY][0], MON_DATA_LEVEL), 100);
    EXPECT_EQ(GetMonData(&gParty[B_ENEMY][1], MON_DATA_SPECIES), SPECIES_WYNAUT);
    EXPECT_EQ(GetMonData(&gParty[B_ENEMY][1], MON_DATA_LEVEL), 10);
}
