...
WILD_BATTLE_TEST("Large exp gains are supported", s32 exp) // #1455
{
    ...
    GIVEN {
        ...
        EXPECT(GetMonData(&gParty[B_PLAYER][0], MON_DATA_LEVEL) > 1);
        EXPECT(GetMonData(&gParty[B_PLAYER][0], MON_DATA_EXP) > 1);
    } FINALLY {
        EXPECT_GT(results[1].exp, results[0].exp);
...
WILD_BATTLE_TEST("Exp Share(held) gives Experience to mons which did not participate in battle")
{
    ...
    GIVEN {
        ...
        if (item == ITEM_EXP_SHARE)
            EXPECT_GT(GetMonData(&gParty[B_PLAYER][1], MON_DATA_EXP), gExperienceTables[gSpeciesInfo[SPECIES_WYNAUT].growthRate][40]);
        else
            EXPECT_EQ(GetMonData(&gParty[B_PLAYER][1], MON_DATA_EXP), gExperienceTables[gSpeciesInfo[SPECIES_WYNAUT].growthRate][40]);
    }
}
