...
DOUBLE_BATTLE_TEST("t1")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET) { Speed(100); Item(ITEM_EJECT_BUTTON); };
        OPPONENT(SPECIES_WOBBUFFET) { Speed(200); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_HEAT_WAVE); SEND_OUT(opponentLeft, 2); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("t5")
{
    GIVEN {
        ...
        ANIMATION(ANIM_TYPE_MOVE, MOVE_BOOMBURST);
    }
}